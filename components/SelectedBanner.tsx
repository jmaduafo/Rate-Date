...
function SelectedBanner({ title, name, setSelect, select }: SelectProp) {
  return (
    ...
    >
      <p className={`${select && select === name ? 'text-myForeground' : 'text-darkText'} w-fit whitespace-nowrap text-[13px] xs:text-[14px] font-normal group-hover:text-myForeground`}>{title}</p>
    </div>
  );
}
